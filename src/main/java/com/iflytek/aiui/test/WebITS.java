...
import com.iflytek.aiui.face.HttpUtil;
...
public class WebITS {	
	...
	// 应用ID（到控制台获取）
	private static final String APPID = "5d2e7371";
	// 接口APIKey（到控制台机器翻译服务页面获取）
	private static final String API_KEY = "285b9d1617ff7869bdeb21b67949641d";
	// 接口APISercet（到控制台机器翻译服务页面获取）
	private static final String API_SECRET = "de6d3f8ebca5b4097ee2c8d2a8412d42";
...
//		System.out.println(itrans(TEXT));
	}

	public static String itrans(String text, String from, String to) throws Exception {
		...
		if (resultMap != null) {
			...
			// 提取出翻译之后的结果
			Map<String, Object> result = (Map<String, Object>) resultData.getData();
			Map<String, Object> trans_result = (Map<String, Object>)result.get("result");
			Map<String, Object> dst = (Map<String, Object>) trans_result.get("trans_result");
			return (String)dst.get("dst");
		} else {
			System.out.println("调用失败！请根据错误信息检查代码，接口文档：https://www.xfyun.cn/doc/nlp/xftrans/API.html");
		}
		return "";
   ...
   public static Map<String, String> buildHttpHeader(String body) throws Exception {
    ...
    private static final String[] LANG = {"cn", "en", "ja", "fr", "ru", "es", "ar"};

	/**
	 * 组装http请求体
	 */	
   public static String buildHttpBody(String text, String from, String to) throws Exception {
	   if(Arrays.binarySearch(LANG, from) == -1){
	   		from = FROM;
	   }
	   if(Arrays.binarySearch(LANG, to) == -1){
		   	to = TO;
	   }
